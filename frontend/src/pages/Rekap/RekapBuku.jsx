...
function RekapBuku() {
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(readings.length / readingsPerPage); i++) {
    pageNumbers.push(i);
  }
  ...
  return (
    <div>
      ...
      <div className="container md:mx-auto px-10 p-20 ">
        <h1 className="text-2xl font-bold mb-4">Rekap Buku</h1>
        <div className="table-fixed p-8 shadow-lg">
                ...
                {currentReadings.map((reading, index) => (
                  <tr key={reading.uuid}>
                    <td className="py-2 px-4 border-b text-center">
                      {indexOfFirstReading + index + 1}
                    </td>
                    <td className="py-2 px-4 border-b">
                      {new Date(reading.date).toLocaleDateString()}
                    </td>
                    <td className="py-2 px-4 border-b">{reading.note}</td>
                    <td className="py-2 px-4 border-b text-center">
                      <Link to={`/EditJurnalBuku/${reading.uuid}`}>
          ...
          <div className="flex justify-between mt-4">
            <span>
              Showing {indexOfFirstReading + 1} to{" "}
